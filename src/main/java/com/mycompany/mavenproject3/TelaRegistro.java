...
import com.mycompany.legacy.SistemaTeatro;
...
public class TelaRegistro extends JFrame {
    ...
    private void registrarUsuario() {
        ...
        try (Workbook workbook = arquivoExcel.exists() ? new XSSFWorkbook(new FileInputStream(arquivoExcel)) : new XSSFWorkbook()) {
            ...
            // Verificação de duplicação (nome e CPF)
            for (int i = 1; i <= sheet.getLastRowNum(); i++) { // Começa de 1 para ignorar o cabeçalho
                Row row = sheet.getRow(i);
                if (row != null) {
                    String nomeExistente = row.getCell(0).getStringCellValue();
                    String cpfExistente = row.getCell(3).getStringCellValue();

                    // Verifica duplicação de nome ou CPF
                    if (nomeExistente.equalsIgnoreCase(nome)) {
                        JOptionPane.showMessageDialog(this, "Nome já registrado. Por favor, insira um nome diferente.", "Erro", JOptionPane.ERROR_MESSAGE);
                        return; // Encerra o método se encontrar duplicação de nome
                    }
                    if (cpfExistente.equalsIgnoreCase(cpf)) {
                        JOptionPane.showMessageDialog(this, "CPF já registrado. Por favor, insira um CPF diferente.", "Erro", JOptionPane.ERROR_MESSAGE);
                        return; // Encerra o método se encontrar duplicação de CPF
                    }
                }
            }

            // Se não houver duplicatas, registra o novo usuário
