...
public class ProjetoIntegrador {
    ...
    // Método para mostrar os assentos de uma sessão específica de uma peça específica
private static void mostrarAssentos(Teatro teatro, Scanner scanner) {
    System.out.println("\nVisualizar Assentos por Sessão e Peça");
    ...
    Peca pecaEscolhida = pecas.get(escolhaPeca - 1);
    ...
    // Encontrar o setor que corresponde à peça escolhida e à sessão escolhida
    Setor setorEscolhido = null;
    for (Setor setor : teatro.getSetores()) {
        if (setor.getNome().startsWith(pecaEscolhida.getNome())) {
            // Verificar se o setor possui a sessão escolhida
            if (setor.getSessoes().contains(sessaoEscolhida)) {
    ...
    if (setorEscolhido != null) {
        System.out.println("\nAssentos disponíveis para a peça " + pecaEscolhida.getNome() + " na sessão " + sessaoEscolhida + ":");
        System.out.println("\nSetor: " + setorEscolhido.getNome());
        setorEscolhido.mostrarAssentos(sessaoEscolhida); // Chama o método mostrarAssentos de Setor passando a sessão escolhida como argumento
    } else {
        System.out.println("Peça ou sessão não encontrada.");
    }
}
